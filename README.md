...
- `com.nablarch.example.client.ProjectClient` に記載の接続先URLを、`http://127.0.0.1:8080/nablarch-example-rest/projects` に変更。  
  Dockerコンテナにデプロイしたアプリの動作確認を行うに辺り、URLのDocker起動時に指定するポート番号の指定と、コンテキストパスの付与をしました。
...
#### ビルド
`mvn package -DskipTests=true` でビルドできます。
...
### 打鍵テストの方法について
`com.nablarch.example.client.ProjectClient` を実行してください。

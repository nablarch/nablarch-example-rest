...
| 変換されたYAMLがスキーマに対して有効である | OK | 変換ツール（nablarch-testing-converter）が例外なく正常終了（BUILD SUCCESS）。コンバーターは内部でスキーマ検証を実施するため正常終了＝スキーマ準拠 | OK | 全12ファイル。setup_tables/expected_tables/list_maps がスキーマ定義どおり（QAレビューで確認済み） |
